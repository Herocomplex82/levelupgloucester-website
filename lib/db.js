  ...
  const { results } = await db
    .prepare(
      `SELECT id, title, event_date, location, price_full_cents, price_half_cents,
              capacity, seats_taken, image_path, (capacity - seats_taken) AS seatsRemaining
       FROM workshop_days
...
export async function insertWorkshopDay(
  db,
  { title, eventDate, location, priceFullCents, priceHalfCents, capacity, image_path = null }
) {
  const { meta } = await db
    .prepare(
      `INSERT INTO workshop_days
        (title, event_date, location, price_full_cents, price_half_cents, capacity, image_path)
       VALUES (?, ?, ?, ?, ?, ?, ?)`
    )
    .bind(title, eventDate, location, priceFullCents, priceHalfCents, capacity, image_path)
    .run();
  return { id: meta.last_row_id };
}

export async function updateWorkshopDayImage(db, { id, image_path }) {
  await db.prepare("UPDATE workshop_days SET image_path = ? WHERE id = ?").bind(image_path, id).run();
}
